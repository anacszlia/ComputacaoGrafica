...
void criar(PPM *ppm, int largura, int altura, RGB cor){
    if (ppm->pixels)
        delete ppm->pixels;
    ...
    for (int i = 0; i < tamanho; i+=3){
        ppm->pixels[i] = cor.r;
        ppm->pixels[i+1]=cor.g;
        ppm->pixels[i+2]=cor.b;
    }
}
...
void setLinhasAleatorias(PPM *ppm,int salt){
    srand(time(NULL));
    for(int j=salt;j<ppm->alt;j+=salt){
...
void copiaArea(PPM *imgE,PPM *imgS,int x1,int y1,int x2,int y2){
